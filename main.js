angular.module('myWork', []).controller('GameController', ['$scope', function($scope) {
...
$(document).ready(function() {   
		...
		$(function() {
		 ...
		 $('.box').on('click', function() {
			  ...
			  //Set up new destination for selected box
					...
					let selectedPieceInfo = select.piece.split('-');
					let color = selectedPieceInfo[0];
					let type = selectedPieceInfo[1];
	...
	let getNextMoves = function(selectedPiece, selectedBox) {
		 let selectedPieceInfo = selectedPiece.split('-');
		 let color = selectedPieceInfo[0];
		 let type = selectedPieceInfo[1];

		 let id = selectedBox.split('-');
		 let i = parseInt(id[1]);
		 let j = parseInt(id[2]);
	...
	let getPawnMoves = function(i, j, color, moves) {
		 var nextMoves = [];
		 for(let index = 0; index < moves.length; index++) {
			  let tI = i + moves[index][0];
			  let tJ = j + moves[index][1];
	...
	let getQueenMoves = function(i, j, color, moves) {
		 var nextMoves = [];
	...
	let getKnightMoves = function(i, j, color, moves) {
		 var nextMoves = [];
	...
	let setPiece = function(box, color, type) {
		 ...
		 let j = parseInt(box.attr('id').charAt(6));
		 if(type === 'pawn') {
			  if( (player === 'black' && j === 7) ||
					(player === 'white' && j === 0)) {
					$('#game').css('opacity', '0.5');

					let option = $('#pawn-promotion-option');
	...
	//Default board state
	let setNewBoard = function(box, i, j) {
		 if(j === 7) {
			  if(i === 0 || i === 7) {
